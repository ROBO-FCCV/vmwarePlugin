...
check_path=${current_dir}/vmware_cert_check.sh
...
function check_mode(){
    if [[ ${current_dir} =~ "/opt/plugin/vmware_plugin" ]];then
        if [[ ${para_num} -ne 0 ]];then
            echo "The uninstall.sh no parameters required."
            exit 0
        fi
    ...
    if [[ ! ${current_dir} =~ "/vmware_plugin" ]];then
        return 0
    fi
    conf_file=${base_dir}/conf/install.conf
    serviceName=`grep vmware_name ${conf_file} | awk -F"=" '{print $2}'`
...
function para_check(){
    if [[ "--service"x != "$str"x && "--all"x != "$str"x ]]; then
