...
class VmwareUser:
    ...
    def vmware_get_pass(self):
        count_pass = 0
        while count_pass < 4 :
            vmware_password = getpass.getpass("Please enter the vCenter password: ")
            if not vmware_password:
                print("The input is null,please enter again!")
                count_pass = count_pass + 1
                continue
            vmware_password2 = getpass.getpass("Please enter the vCenter password again: ")
    ...
    def vmware_update_infor(self):
        ...
        script1 = 'sed -n "/%s/,/vmwareId/=" %s |tail -n1' % (infor['old_ip'], self.file_path)
    ...
    def vmware_delete_infor(self, delte_ip):
        script1 = 'sed -n "/%s/,/vmwareId/=" %s |tail -n1' % (delte_ip, self.file_path)
