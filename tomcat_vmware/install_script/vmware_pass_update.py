...
# auth
pass_path = "/opt/plugin/vmware_plugin/tomcat/webapps/" \
            "vmware/WEB-INF/classes/login.yml"
log_path = "/var/log/plugin/vmware_plugin/pwd.log"
nginx_ip = commands.getoutput('sudo /usr/bin/get_info.py manage_ip')
conf = {
    "update_pass_uri": "/dfxmgt/storepass",
    "check_pwd_uri": '/usermgt/policydata/weakpwdcheck'
}
...
class UpdatePassword:
    ...
    def vmware_get_pass(self):
        count_pass = 0
        while count_pass < 4 :
            vmware_password = getpass.getpass("Please enter the new password: ")
            ...
            if self.checkpass_contain_upper(vmware_password) is False:
                print 'The new password must contain at least one uppercase letter.'
                continue
            if self.checkpass_contain_lower(vmware_password) is False:
                print 'The new password must contain at least one lowercase letter.'
                continue
            ...
            if self.checkpass_contain_symbol(vmware_password) is False:
                print 'The new password must contain at least one of the special characters inside the brackets(!\'#$%&\'()*+_,./:;<=>?@[]^{_|}~).Please re-enter a new password.'
                continue
            if self.check_pwd_data(vmware_password) is False:
                print 'Password too weak.'
                continue
            vmware_password2 = getpass.getpass("Please enter the new password again: ")
    ...
    def update_vmware_newpass(self):
        ...
        try:
            ...
            if int(get_data.get("code")) == 0:
                script1 = 'sed -i 4"c \  %s" %s' % (get_data.get("data"), self.pass_path)
